...
from primers_and_melting_temperature import readDNAsequence
...
def translate(dna_sequence):
    ...
    #Cleaning non-triplet codons in reading frames 2 and 3
    f2_listed_codons = f2_listed_codons[:-1]
    f3_listed_codons = f3_listed_codons[:-1]
    r2_listed_codons = r2_listed_codons[:-1]
    r3_listed_codons = r3_listed_codons[:-1]
    ...
    #Stringed translated seqeuences
    stringed_translated_sequence_f1 = ''.join(translated_sequence_f1)
    stringed_translated_sequence_f2 = ''.join(translated_sequence_f2)
    stringed_translated_sequence_f3 = ''.join(translated_sequence_f3)
    stringed_translated_sequence_r1 = ''.join(translated_sequence_r1)
    stringed_translated_sequence_r2 = ''.join(translated_sequence_r2)
    stringed_translated_sequence_r3 = ''.join(translated_sequence_r3)
    ...
    #Build dictionary
    amino_acid_dict = {}
    amino_acid_dict ['f1'] = stringed_translated_sequence_f1
    amino_acid_dict ['f2'] = stringed_translated_sequence_f2
    amino_acid_dict ['f3'] = stringed_translated_sequence_f3
    amino_acid_dict ['r1'] = stringed_translated_sequence_r1
    amino_acid_dict ['r2'] = stringed_translated_sequence_r2
    amino_acid_dict ['r3'] = stringed_translated_sequence_r3
...
def openReadingFrame(sequence):
    ...
    and the first STOP codon that follows it (excluded). The stop codon is represented by 
    ...
    if 'M' in sequence and '*' in sequence:
        ...
        stop_position = sequence.index('*')
        orf = ''.join(sequence[met_position: stop_position])
        return (orf)
    else:
        return ("")
...
def writeFASTA (sequence, description, filename):
    ...
    #Line breaking at 60 characters
    linew=60
    pos=0
